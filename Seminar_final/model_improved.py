...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        K = self.k_proj(x)
        V = self.v_proj(x)
        output, weights = scaled_dot_product_attention(Q, K, V, mask=mask, dropout_layer=self.dropout)
        return output, weights
...
class TransformerEncoderBlock(nn.Module):
    def __init__(self, d_model: int, d_ff: int, dropout: float = 0.1):
        super().__init__()
        self.self_attention = SelfAttention(d_model, dropout)
...
class TransformerClassifier(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, d_ff: int, max_len: int, num_classes: int, dropout: float = 0.1):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, d_model)
        self.pos_encoding = PositionalEncoding(d_model=d_model, max_len=max_len)
        self.encoder = TransformerEncoderBlock(d_model=d_model, d_ff=d_ff, dropout=dropout)
